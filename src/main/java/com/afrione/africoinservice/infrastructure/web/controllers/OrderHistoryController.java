...
public class OrderHistoryController {
    ...
    public ApiResponseJSON<PagedResponse<OrderHistoryResponse>> getOrderHistory(
        ...
        PagedResponse<OrderHistoryResponse> response = orderHistoryUseCases.getOrderHistory(orderType, startDate, endDate, corridor, status, page, size);
    ...
    public ApiResponseJSON<OrderHistoryResponse> getOrderDetail(@PathVariable String orderId, @PathVariable @Pattern(regexp = "OFF_RAMP|ON_RAMP") String orderType) {
        OrderHistoryResponse response = orderHistoryUseCases.getOrderDetail(orderId, orderType);
    ...
    public ApiResponseJSON<List<OrderSummaryResponse>> getOrderSummary(
            ...
            @RequestParam(required = false) @DateTimeFormat(pattern = "dd/MM/yyyy") @PastOrPresent LocalDate startDate,
            @RequestParam(required = false) @DateTimeFormat(pattern = "dd/MM/yyyy") @PastOrPresent LocalDate endDate) {
        List<OrderSummaryResponse> response = orderHistoryUseCases.getOrderSummary(orderType, corridor, startDate, endDate);
